...
class SampleFrame extends JFrame implements EWrapper {
	...
	private JPanel createButtonPanel() {
		...
		butRequestSecurityDefinitionOptionParameters
				.addActionListener(e -> onRequestSecurityDefinitionOptionParameters());
	...
	private void onReqTickByTickData() {
		...
		if (m_orderDlg.m_rc) {
			m_client.reqTickByTickData(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.tickByTickType(),
					m_orderDlg.numberOfTicks(), m_orderDlg.ignoreSize());
		}
	}
	...
	private void onStopMonitorTransaction() {
		...
		Iterator iter = m_contractList.entrySet().iterator();
		while(iter.hasNext()) {
		    Map.Entry entry = (Map.Entry)iter.next();

		    conid = (Integer)entry.getKey();
		    contact = (Contract)entry.getValue();
		    m_client.cancelMktData(conid);
		}
	...
	private void onReqHistoricalTicks() {
		...
		if (m_orderDlg.m_rc) {
			m_client.reqHistoricalTicks(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.startDateTime(),
					...
					m_orderDlg.useRTH(), m_orderDlg.ignoreSize(), m_orderDlg.options());
		}
	}
	...
	private void onReqMktData() {
		...
		// req mkt data
		m_client.reqMktData(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.m_genericTicks,
				m_orderDlg.m_snapshotMktData, m_orderDlg.m_reqSnapshotMktData, m_mktDataOptions);
	}
	...
	private void onReqRealTimeBars() {
		...
		m_client.reqRealTimeBars(m_orderDlg.id(), m_orderDlg.contract(),
				5 /* TODO: parse and use m_orderDlg.m_barSizeSetting */, m_orderDlg.whatToShow(),
				m_orderDlg.useRTH() > 0, m_realTimeBarsOptions);
	}
	...
	private void onHeadTimestamp() {
		...
		// req head timestamp
		m_client.reqHeadTimestamp(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.whatToShow(), m_orderDlg.useRTH(),
				m_orderDlg.formatDate());
	}
	...
	private void onHistoricalData() {
		...
		// req historical data
		m_client.reqHistoricalData(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.backfillEndTime(),
				m_orderDlg.backfillDuration(), m_orderDlg.barSizeSetting(), m_orderDlg.whatToShow(),
	...
	private void onFundamentalData() {
		...
		m_client.reqFundamentalData(m_orderDlg.id(), m_orderDlg.contract(), /* reportType */ m_orderDlg.whatToShow(),
				null);
	}
	...
	private void onExerciseOptions() {
		...
		// cancel order
		m_client.exerciseOptions(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.m_exerciseAction,
	...
	private void onCalculateImpliedVolatility() {
		...
		m_client.calculateImpliedVolatility(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.m_order.lmtPrice(),
				m_orderDlg.m_order.auxPrice(), null);
	}
	...
	private void onCalculateOptionPrice() {
		...
		m_client.calculateOptionPrice(m_orderDlg.id(), m_orderDlg.contract(), m_orderDlg.m_order.lmtPrice(),
				m_orderDlg.m_order.auxPrice(), null);
	}
	...
	private boolean filterTickType(int field)
	{
		if (
				m_contractList.size() == 0 
				|| m_positionList.size() == 0 
				|| m_avgCostList.size() == 0 
				) {
			return Boolean.FALSE;
		}
		if (
//				TickType.get(field) == TickType.LAST_SIZE
				//TickType.get(field) == TickType.LAST
				TickType.get(field) == TickType.ASK
//				|| TickType.get(field) == TickType.ASK_SIZE
				|| TickType.get(field) == TickType.BID
				//|| TickType.get(field) == TickType.MARK_PRICE
//				|| TickType.get(field) == TickType.BID_SIZE
				) {
			return Boolean.TRUE;
		}
...
//				);
		...
		} else if (contract.secType() == SecType.STK) {
	...
	public void tickGeneric(int tickerId, int tickType, double value) {
		...
		if (filterTickType(tickType) == Boolean.FALSE) {
			return;
		}
	...
	public void historicalData(int reqId, Bar bar) {
		String msg = EWrapperMsgGenerator.historicalData(reqId, bar.time(), bar.open(), bar.high(), bar.low(),
	...
	public void historicalTicksBidAsk(int reqId, List<HistoricalTickBidAsk> ticks, boolean done) {
		...
		for (HistoricalTickBidAsk tick : ticks) {
			msg.append(EWrapperMsgGenerator.historicalTickBidAsk(reqId, tick.time(), tick.mask(), tick.priceBid(),
	...
	public void historicalTicksLast(int reqId, List<HistoricalTickLast> ticks, boolean done) {
		...
		for (HistoricalTickLast tick : ticks) {
			msg.append(EWrapperMsgGenerator.historicalTickLast(reqId, tick.time(), tick.mask(), tick.price(),
	...
	public void tickByTickAllLast(int reqId, int tickType, long time, double price, int size, TickAttr attribs,
		...
		String msg = EWrapperMsgGenerator.tickByTickAllLast(reqId, tickType, time, price, size, attribs, exchange,
				specialConditions);
